...
@section('page-title')
    <h1 class="text-2xl font-bold text-slate-900">Nuevo Deal</h1>
@endsection

@section('content')
    <x-card title="Crear deal">
        <form action="{{ route('crm.deals.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div class="grid gap-5 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input id="title" name="title" type="text" maxlength="255" required value="{{ old('title') }}" class="w-full rounded-md border px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none {{ $errors->has('title') ? 'border-red-500' : 'border-slate-300' }}">
                    @error('title')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input id="value" name="value" type="number" step="0.01" min="0" required value="{{ old('value') }}" class="w-full rounded-md border px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none {{ $errors->has('value') ? 'border-red-500' : 'border-slate-300' }}">
                    @error('value')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="pipeline_stage_id" name="pipeline_stage_id" class="w-full rounded-md border px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none {{ $errors->has('pipeline_stage_id') ? 'border-red-500' : 'border-slate-300' }}">
                        @foreach ($stages as $stage)
                            <option value="{{ $stage->id }}" @selected((string) old('pipeline_stage_id', optional($stages->firstWhere('is_default', true))->id) === (string) $stage->id)>{{ $stage->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input id="expected_close_date" name="expected_close_date" type="date" value="{{ old('expected_close_date') }}" class="w-full rounded-md border px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none {{ $errors->has('expected_close_date') ? 'border-red-500' : 'border-slate-300' }}">
                ...
                <div>
                    ...
                    <select id="status" name="status" class="w-full rounded-md border px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none {{ $errors->has('status') ? 'border-red-500' : 'border-slate-300' }}">
                        ...
                        <option value="won" @selected(old('status') === 'won')>Ganado</option>
                        <option value="lost" @selected(old('status') === 'lost')>Perdido</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
