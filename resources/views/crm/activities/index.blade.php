...
@section('page-title')
    <div class="flex flex-wrap items-center justify-between gap-3">
        <h1 class="text-2xl font-bold text-slate-900">Actividades de {{ $customer->name }}</h1>
        <a href="{{ route('crm.customers.activities.create', $customer) }}" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-indigo-700">
            Nueva actividad
        </a>
    </div>
@endsection
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left font-semibold text-slate-600">Deal relacionado</th>
                            ...
                            <th class="px-4 py-3 text-left font-semibold text-slate-600">Acción</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse ($activities as $activity)
                            ...
                            <tr x-show="filter === 'all' || filter === '{{ $activity->type->value }}'">
                                ...
                                <td class="px-4 py-3">
                                    @if (! $activity->completed_at)
                                        <form action="{{ route('crm.customers.activities.complete', [$customer, $activity]) }}" method="POST">
                                            @csrf
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-5 text-center text-slate-500">No hay actividades registradas</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
